/*****************************************************************************************
Program: Implementacja progowania Niblack'a z wykorzystaniem wielo¹tkowoœci
Autor: Lech Napiera³a, Marcin Caryk
Parametry wejœciowe:
...
void Nib(C_Image_Container* owe, double w, double k, double n, C_Image_Container* owy, C_Error* perr);
...
//	konwersja danych z matlaba na postaæ akceptowalna przez moj¹ funkcjê
	...
	//fprintf(plik, "Dochodzê do wywo³ania Funkcji\n");
	Nib(&in1, *w, *k, *N, &out, &errr);	// wywo³anie funkcji
...
//	konwersja danych z mojej funkcji do matlaba
	//fprintf(plik, "Dochodzê do zamykania pliku i eksportowania do matlaba\n");
	//fclose(plik);
...
unsigned int WINAPI NIBLACK(void* param)
{
	struct SAV *s;
	s = (struct SAV *)param;
	...
	unsigned long w2;
	//fprintf(plik, "W¹tek, zmienne zadeklarowane\n");
	imin = s->imin;
	imax = s->imax;
	jmin = s->jmin;
	jmax = s->jmax;
...
void Nib(C_Image_Container* owe, double w, double k, double n, C_Image_Container* owy, C_Error* perr)
{
	...
	if(w < owe->_cols && w < owe->_rows) //Sprawdzamy czy obraz jest wiêkszy od maski, inaczej dalsze liczenie nie ma sensu
	{
		...
		for(i = 0; i < n; i++)
		{
			hThread[i] = (HANDLE)_beginthreadex(NULL, 0, NIBLACK, (void*)&sav[i], 0, NULL);
		}
